...
public class MainUpView extends FrameLayout {
	private static final String TAG = MainUpView.class.getSimpleName();
	...
	public void setUpImageRes() {
		Drawable shape = context.getResources().getDrawable(R.drawable.home_main_focus);
		setUpRectDrawable(shape);
	}
	...
	public void setDrawShadowPadding(int size) {
		setDrawShadowPadding(new Rect(size, size, size, size));
	}
	 ...
	 * 比如 res/drawble/white_shadow.9.png的图片，边距就差很多.
	 */
	public void setDrawShadowPadding(Rect rect) {
		if (mEffectBridge != null) {
